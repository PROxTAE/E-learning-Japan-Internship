...
export function SessionDeleteModal({ session, isOpen, onClose, onConfirm, isDeleting }: SessionDeleteModalProps) {
  const { t } = useLang();
            ...
            <div className="w-full max-w-md bg-white dark:bg-[#1a1a2e] rounded-3xl shadow-2xl border border-gray-200 dark:border-white/10 pointer-events-auto overflow-hidden">
              ...
              <div className="p-6 space-y-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-base font-bold text-gray-900 dark:text-white">{t.sessionHistory.deleteSession}</h3>
                    <p className="text-xs text-gray-500 dark:text-default-400">{t.quizList.deleteUndone}</p>
                  </div>
                </div>
                ...
                <div className="px-4 py-3 rounded-xl bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10">
                  ...
                  <p className="text-xs text-gray-500 dark:text-default-400 mt-0.5">
                    {session.studentCount} students · {session.stats?.averageScore ?? 0}% avg score
                  </p>
                </div>

                <p className="text-sm text-gray-600 dark:text-default-400">
                  {t.sessionHistory.deleteConfirm}
                </p>
