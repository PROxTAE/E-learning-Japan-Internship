...
import { ChevronLeft, Trophy, Target, Star, RotateCcw } from "lucide-react";
...
export default function PlayQuizPage() {
      ...
      <div className="quiz-bg fixed inset-0 flex flex-col items-center justify-center p-4">
        <Card className="w-full max-w-md bg-white/10 backdrop-blur-md border border-white/20 rounded-3xl">
          <CardContent className="text-center py-12 px-8 flex flex-col items-center gap-6">
            ...
            <Button 
              className="w-full h-14 rounded-2xl font-bold bg-white text-violet-600 shadow-xl" 
              onPress={() => router.push("/")}
            >
              Back to Home
            </Button>
  ...
  const handleNext = () => {
    ...
    setSelectedAnswers({ ...selectedAnswers, [currentQuestion.id]: currentSelection });
        ...
        <div className="relative min-h-full flex flex-col items-center justify-center px-4 py-10">
          <div className="w-full max-w-md flex flex-col gap-6 mx-auto">
            ...
            >
              ...
              <div>
                <h1 className="text-2xl font-extrabold text-white tracking-tight">Quiz Submitted!</h1>
                <p className="text-white/55 text-sm mt-1">Thank you for participating.</p>
              </div>
            </motion.div>
            ...
            <Card className="w-full rounded-[32px] border-0 shadow-2xl overflow-hidden bg-white dark:bg-zinc-900">
              ...
              <CardContent className="flex flex-col items-center gap-8 py-10 px-8">
                <div className="text-center space-y-2">
                  ...
                  <h3 className="text-xl font-bold text-zinc-800 dark:text-white">Well Done!</h3>
                  <p className="text-zinc-500 dark:text-zinc-400">
                    Your answers for <span className="font-semibold text-violet-500">{quiz.title}</span> have been sent to the teacher.
                  </p>
                </div>

                <div className="grid grid-cols-2 gap-4 w-full">
                  <div className="bg-zinc-50 dark:bg-zinc-800/50 p-4 rounded-2xl border border-zinc-100 dark:border-zinc-700/50 flex flex-col items-center gap-1">
                    <span className="text-2xl font-black text-zinc-800 dark:text-white">{quiz.questions.length}</span>
                    <span className="text-[10px] font-bold uppercase tracking-wider text-zinc-400">Questions</span>
                  </div>
                  <div className="bg-violet-50 dark:bg-violet-900/20 p-4 rounded-2xl border border-violet-100 dark:border-violet-800/50 flex flex-col items-center gap-1">
                    <span className="text-2xl font-black text-violet-600 dark:text-violet-300">Sent</span>
                    <span className="text-[10px] font-bold uppercase tracking-wider text-violet-400">Status</span>
                  </div>
                </div>

                <Button 
                  className="w-full py-7 rounded-2xl font-black text-lg bg-gradient-to-r from-violet-600 to-purple-600 text-white shadow-xl hover:shadow-violet-500/30"
                  onPress={() => router.push("/")}
                >
                  Back to Home
                </Button>
          ...
          >
            <Card className="w-full rounded-[32px] border-0 shadow-2xl overflow-hidden bg-white dark:bg-zinc-900">
              ...
              <CardContent className="text-center py-10 px-8 flex flex-col items-center gap-6">
                ...
                <button 
                  onClick={() => router.push("/")}
                  className="text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-200 text-sm font-semibold transition-colors"
                >
      ...
      <div className="relative min-h-full flex flex-col items-center justify-start px-4 sm:px-6 md:px-8 py-6 sm:py-10 w-full max-w-sm sm:max-w-md md:max-w-lg lg:max-w-xl mx-auto">
        ...
        >
          ...
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="flex items-center gap-1.5 bg-white/20 dark:bg-white/10 backdrop-blur-md border border-white/25 rounded-full px-3 py-1.5">
              <span className="text-sm">📚</span>
              <span className="text-white font-bold text-xs uppercase tracking-tight">Playing</span>
            </div>
          </div>
              ...
              <Card className="w-full rounded-[32px] border-0 shadow-2xl bg-white dark:bg-zinc-900 overflow-hidden">
                  ...
                  {question.imageUrl && (
                    <div className="mt-6 rounded-2xl overflow-hidden border border-zinc-100 dark:border-zinc-800">
                      ...
                      <img 
                        ...
                        alt="Question" 
                        className="w-full max-h-64 object-contain bg-zinc-50 dark:bg-black/20"
                      />
                    </div>
                  )}
                </CardContent>
                ...
                <CardContent className="px-6 sm:px-8 pb-8 sm:pb-10 pt-4">
                  <RadioGroup
                    ...
                    onChange={(val) => setCurrentSelection(val)}
